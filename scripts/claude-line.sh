...
while true; do
  ...
  (
    ...
    if tmux has-session -t "$TMUX_SESSION" 2>/dev/null; then
      ...
      QUEUE_FILE="$HOME/.claude/channels/line/runtime/line-lobster-queue.jsonl"
      ...
      if [ "${QUEUE_SIZE:-0}" -gt 0 ]; then
        ...
        COOLDOWN_FILE="$HOME/.claude/channels/line/runtime/line-trigger-cooldown-claude-line"
        NOW_MS=$(date +%s%3N)
        LAST_MS=0
