...
description: |
  ...
  觸發：放音樂、聲音太大、現在播什麼、sonos、音響。
  不觸發：只想查歌名（用搜尋）、要播 YouTube（Sonos 不支援）。
...
- **禁止手刻 DIDL XML**：直接複製 `~/.openclaw/workspace/scripts/music-qobuz.sh` 裡的 Python 段當模板，不要自己從頭寫

### 腳本
| 平台 | 腳本 | 用法 |
|------|------|------|
| Qobuz | `~/.openclaw/workspace/scripts/music-qobuz.sh` | `music-qobuz.sh play "query" --name "書房"`（⚠️ 單首用，多首用 soco） |
| TIDAL | `~/.openclaw/workspace/scripts/music-tidal.sh` | `music-tidal.sh play "query" --name "書房"` |
| Apple Music | `~/.openclaw/workspace/scripts/music-apple.sh` | `music-apple.sh play "query" --name "書房"` |
...
- 公開 URL：`https://vault.life-os.work/90_System/Inbox/album-art/{artist}-{album}.jpg`
- 使用 `[[media_player: 曲名 | 歌手 | 來源 | vault封面URL]]`（4 參數，無按鈕）
- ⚠️ 不要帶第 5 個參數（playing/paused），否則會出現醜按鈕
- 禁止使用 curl Push 送 Flex（浪費 Push 額度）
- 此功能依賴 patch：`bash ~/Documents/Life-OS/scripts/patch-openclaw-media-player.sh`（升級 OpenClaw 後需重跑）
...
COVER_URL="https://vault.life-os.work/90_System/Inbox/album-art/${SAFE_NAME}.jpg"
```
...
| 服務 | 官方來源 URL | 本機路徑 |
|------|--------------|----------|
| Spotify | `https://open.spotifycdn.com/cdn/images/favicon32.b64ecc03.png` | `~/.openclaw/skills/sonoscli/assets/spotify.png` |
| TIDAL | `https://tidal.com/favicon.ico` | `~/.openclaw/skills/sonoscli/assets/tidal.png` |
| Qobuz | `https://play.qobuz.com/apple-touch-icon.png` | `~/.openclaw/skills/sonoscli/assets/qobuz.png` |
| Apple Music | `https://music.apple.com/favicon.ico` | `~/.openclaw/skills/sonoscli/assets/applemusic.png` |
