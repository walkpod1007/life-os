...
## Agent 派工分工（2026-04-15 定版）

- **Codex CLI**：本機盤查、程式碼調查、pre-execution 審查、紅隊——額度充足，是本機任務的主力
- **Gemini CLI**：只用於網路查詢和 YouTube 相關任務（抓字幕、搜尋最佳實踐等）
- **Claude Opus (subagent)**：執行複雜實作任務
- **Claude Sonnet (主 session)**：協調、設計、派工

不要把本機盤查派給 Gemini——它適合搜網路，不適合讀本機程式碼。
