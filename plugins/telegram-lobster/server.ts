#!/usr/bin/env bun
// ═══════════════════════════════════════════════════════════════════════════════
// LOBSTER FORK — DO NOT USE OFFICIAL CACHE VERSION
// Base: telegram plugin 0.0.4
// Customizations: boost_keywords (every-turn), mode switching, turn_protocol,
//                 STATE.md/flag.md injection, extractKeywords removal
// Note: Vault JIT intentionally omitted — semantic search hook covers this
// Fork date: 2026-03-25
// Upstream check: diff against latest cache version before merging
// ═══════════════════════════════════════════════════════════════════════════════
/**
...
function readCited(): string {
  try {
    const citedFile = join(homedir(), 'Documents', 'Life-OS', 'cited.md')
    const text = readFileSync(citedFile, 'utf8')
    const lines = text.split('\n').filter(l => l.trim() && !l.startsWith('##'))
    if (lines.length === 0) return ''
    // Filter: only show entries from last 3 days
    const now = new Date()
    const threeDaysAgo = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)
    const recent = lines.filter(l => {
      const dateStr = l.trim().slice(0, 10)
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: 'boost_keywords',
      description: 'Called every turn. Scan last 10 turns of conversation. Extract 0-3 anchors. For each anchor, specify c (channel: mood | focus | need | thread | stance) and k (keyword or phrase). mood=emotional state, focus=topic being worked on, need=what user wants/needs [inferred], thread=ongoing topic/project, stance=behavioral pattern, taste=品味偏好(影視、音樂、食物、衣著、議題、價值觀、娛樂)— AI主動識別,非每輪必填. Semantic dedup: if new keyword is semantically similar to an existing entry in the same channel, return the existing term (to promote it in LRU) instead of adding a new one. Pass [] if nothing changed. MUST be called every turn.',
      inputSchema: {
        type: 'object',
        properties: {
          keywords: {
            ...
            description: 'Array of 0-3 channel-tagged anchors, e.g. [{"c":"focus","k":"flag.md 5-channel"},{"c":"mood","k":"focused"}]',
          },
        },
        required: ['keywords'],
      },
...
async function handleInbound(
  ...
  if (text.trim() === '/restart') {
    void bot.api.sendMessage(chat_id, '🔄 重啟中，稍後見…').catch(() => {})
...
boost_keywords: scan last 10 turns, update 0-3 anchors with channel tag.
